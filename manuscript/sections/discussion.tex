...
attribution. The five datasets here span two drugs, three biological
scales, and a 14-patient cohort, chosen to stress-test reproducibility
rather than to maximise single-cohort significance. This reframes the
question from ``does drug tolerance produce topological cyclicity?''
to ``is topological cyclicity a reproducible cross-system phenotype?''
Our answer is yes: max $H_1$ increases
...
A less obvious but biologically informative finding is that the
topological signature replicates across cell line and PDX with higher
fidelity than the underlying gene programs. Cell-line top-connected
genes are dominated by canonical EMT markers
(\emph{TPM1}, \emph{KRT8}, \emph{KRT19}); PDX top genes form a broader
mesenchymal/secretory signature
(\emph{S100A4}, \emph{LGALS3}, \emph{COL1A1}, \emph{AGR2}). The
11-gene overlap between the two top-100 lists is highly significant
(hypergeometric $p = 1.25 \times 10^{-11}$) but a minority of either
list. We interpret this as evidence that plasticity is realised
through partially different gene repertoires depending on
microenvironment, consistent with context-dependent EMT programs
\citep{dongre2019}. The topological statistic abstracts over these
specific gene choices, making it a more stable cross-system phenotype
than any individual gene signature.

\subsection*{Relationship to existing literature}

Our framework complements \citet{oren2021}, who used Watermelon
lineage tracing to establish PC9 DTP cycling subpopulations, and
\citet{aissa2021}, who identified discrete DTP states in GSE134839.
It detects cyclic behaviour directly from scRNA-seq without
lineage-tracing infrastructure, and extends the signal to PDX data.
Relative to \citet{rizvi2017} scTDA on mouse ES differentiation, we
extend the framework to perturbation-response and add cell-cycle /
permutation-null controls absent from the original.
...
inter-state cycling further provides a conceptual basis for the
partial clinical efficacy of TKI rechallenge after a drug holiday in
EGFR-mutant NSCLC: cells that survived prior treatment can revert to
drug-sensitive states. The cross-scale reproducibility of the
signature argues that max $H_1$ is a quantitative phenotype for
cell-state plasticity, suitable for benchmarking drug responses
across labs and likely to generalise to adjacent perturbation-response
settings.
